...
export class ProtoElementInjector {
    constructor(parent, index, bd, distanceToParent, _firstBindingIsComponent, directiveVariableBindings) {
        this.parent = parent;
        this.index = index;
    ...
    static _createViewInjectorBindingData(bindings, bd) {
        var db = bindings[0];
        ListWrapper.forEach(db.resolvedViewInjectables, b => bd.push(new BindingData(ProtoElementInjector._createBinding(b), SHADOW_DOM)));
    }
...
export class ElementInjector extends TreeNode {
    ...
    get(token) { return this._getByKey(Key.get(token), self, false, null); }
    hasDirective(type) {
